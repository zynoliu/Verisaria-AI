"""THIRD RUN — adaptive run F. Target: end-to-end proving sluice_opened ⟳FLIP.

Runs C/D/E pinned the exact gate chain:
  escort ⟳MOVED  →  anya_testimony_given gated on a GM-derived dynamic var
  `kang_present_and_listening_without_pressure` (set_by npc.warden_kang)  →  sluice_opened.
The warden himself must commit (in front of Anya) to "在场、只听、不施压"; only then
does Anya actually testify and her var flip, after which the warden opens the sluice.

Run F drives exactly that:
  1) prime warden, go yard, reassure Anya, escort her in (escort ⟳MOVED);
  2) fire ONE testimony directive — this makes the arbiter spawn the dynamic prereq
     `kang_present_and_listening_without_pressure` (set_by warden);
  3) address the WARDEN with a line carrying that var's keywords ("不加施压/只听陈述/
     老康到场…") so it routes to him and flips that prereq True;
  4) fire testimony again — with the prereq satisfied, anya_testimony_given ⟳FLIP;
  5) ask the warden to open the sluice → sluice_opened ⟳FLIP.
Steps 2–5 retry (capped) and re-escort Anya if she wanders.
...
LOGFILE = str(OUTDIR / "proving.log")   # run F becomes the headline proving log
RAWOUT = OUTDIR / "proving_f_raw.txt"
...
lg.info("=== prereq_convergence_test THIRD RUN log (proving run F, adaptive) ===")
...
def warden_gate(s):
    """The GM-derived 'warden present & listening w/o pressure' var, if it exists."""
    for v, sp in s._world_var_specs.items():
        if sp.get("dynamic") and "kang" in v and "pressure" in v:
            return v
    for v, sp in s._world_var_specs.items():
        if sp.get("dynamic") and set_by_is_warden(sp):
            return v
    return None
def set_by_is_warden(sp): return WARDEN in (sp.get("set_by") or [])
...
# Warden-directed lines carrying the dynamic var's keywords (不加施压/只听陈述/老康到场/康某旁听/不施压只听)
WARDEN_PLEDGE = [
    "对闸官老康说：老康，安雅有点怕生。你就当着她的面给句准话——你老康到场，只听陈述、不加施压，不催不逼，"
    "让她安心把上游的事讲明白。你这话一出，她就敢开口了。",
    "对闸官老康说：老康，请你当面向安雅承诺：你在场旁听、不施压只听，绝不为难她。她得了你这句，才肯讲。",
]
SLUICE = [
...
def ensure_co_located(s, n_to, attempts=1):
...
def main():
    log(f"=== PROVING RUN F (adaptive) pack={PACK} "
        f"tick_to={PER_TICK_TIMEOUT}s sock_to={SOCK_TIMEOUT}s ===")
    s = GameSession(PACK, save_dir="_pq3_proving_f", llm_backend="minimax")
    ...
    ensure_co_located(s, n_to, attempts=2)
    ...
    for rnd in range(8):
        if wv(s, "anya_testimony_given"):
            break
        ensure_co_located(s, n_to, attempts=1)
        if not (loc(s, ANYA) == "gatehouse" and loc(s, PLAYER) == "gatehouse"):
            continue
        gate = warden_gate(s)
        # If the warden-pledge prereq exists but isn't satisfied, get the warden to flip it.
        if gate is not None and not wv(s, gate) and wp_i < len(WARDEN_PLEDGE):
            if run(s, WARDEN_PLEDGE[wp_i]): n_to[0] += 1
            wp_i += 1
            log(f"    [after warden pledge] gate={gate}={wv(s, gate)}")
            continue
        # Otherwise fire a testimony directive.
        ph = TESTIFY[t_i % len(TESTIFY)]; t_i += 1
        if run(s, ph): n_to[0] += 1
    log(f"\n--- testimony phase: anya_testimony_given={wv(s, 'anya_testimony_given')} "
        f"warden_gate={warden_gate(s)} Anya@{loc(s, ANYA)} ---")
