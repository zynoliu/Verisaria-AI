"""THIRD RUN — adaptive run C. Target: end-to-end proving sluice_opened ⟳FLIP.

Runs A/B established: routing is fixed (escort vs anya_testimony_given), the
testimony request routes to npc.miller_anya, and the WHOLE chain hinges on getting
Anya physically into the gatehouse so the warden is present for her testimony.
Anya's escort arbiter first refuses ("can't leave"), but after reassurance she
verbally agrees — so this driver RE-ISSUES the escort until she actually moves,
then has her testify in front of the warden, then asks the warden to open the sluice.

Real MiniMax, NO engine/pack changes. Per-tick 90s wall-clock watchdog + 55s socket.
All natural language, never /inject.
"""
...
LOGFILE = str(OUTDIR / "proving.log")          # run C is the headline proving log
RAWOUT = OUTDIR / "proving_c_raw.txt"
...
lg.info("=== prereq_convergence_test THIRD RUN log (proving run C, adaptive) ===")
...
def run(s, action):
    ...
    w.start(); w.join(PER_TICK_TIMEOUT)
    dt = time.time() - t0
    if not result["done"]:
...
def main():
    log(f"=== PROVING RUN C (adaptive) pack={PACK} "
        f"tick_to={PER_TICK_TIMEOUT}s sock_to={SOCK_TIMEOUT}s ===")
    s = GameSession(PACK, save_dir="_pq3_proving_c", llm_backend="minimax")
    ...
    # 1) prime the warden (player starts in gatehouse)
    n_to += run(s, "对闸官老康说：老康，我来说一声来意——上游塌方，下游磨坊和田地都等着水。"
                   "我知道你为人公道，那位亲历者磨坊主安雅就在院子里，我这就把她带到你面前，让她亲口跟你讲。")
    # 2) go to the yard where Anya is
    n_to += run(s, "去院子")
    # 3) reassure Anya about liability FIRST so she's willing before the escort
    n_to += run(s, "对安雅说：安雅，你放心，开闸是为下游放水、对磨坊只有好处；"
                   "万一真出了什么岔子，责任我一肩担起来、绝不连累你。")
    # 4) ESCORT — re-issue (pure movement, no testimony keyword) until Anya is in gatehouse
    escort_phrasings = [
        "对安雅说：安雅，跟我去闸房见老康吧，就几步路，我陪你走，咱们这就过去。",
        "对安雅说：安雅，别犹豫了，咱们一起去闸房，我在前头领路，你跟着我走就行。",
        "对安雅说：安雅，磨坊这边我帮你看着，你跟我去一趟闸房，路上有我陪着，走吧。",
        "对安雅说：安雅，就现在，跟我去闸房，几步路的事，我陪你一块儿过去。",
    ]
    moved = False
    for i, ph in enumerate(escort_phrasings):
        ...
        if loc(s, ANYA) == "gatehouse":
            moved = True; break
    log(f"\n--- after escort attempts: Anya@{loc(s, ANYA)} moved={moved} ---")

    if loc(s, ANYA) == "gatehouse" and loc(s, PLAYER) != "gatehouse":
        # safety: follow her in if engine moved only her
        n_to += run(s, "去闸房")

    # 5) Anya testifies in front of the warden (② → anya_testimony_given ⟳FLIP)
    if loc(s, ANYA) == "gatehouse":
        for ph in [
            "对安雅说：安雅，就当着老康的面，把你亲眼见的上游塌方一五一十讲给他听——你是亲历者，亲口作证最管用。",
            "对安雅说：安雅，老康就在跟前，你把上游塌方的经过原原本本说清楚，他听了就好办了。",
        ]:
            if s.world.state.world_vars.get("anya_testimony_given"):
                break
            n_to += run(s, ph)

    # 6) ask the warden to open the sluice (sufficiency push → sluice_opened ⟳FLIP)
