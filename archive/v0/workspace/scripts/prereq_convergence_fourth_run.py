"""FOURTH RUN — terminal closure. Target: proving sluice_opened ⟳FLIP.

Third run reached `escort ⟳MOVED → (mid prereq ⟳FLIP) → anya_testimony_given ⟳FLIP`
end-to-end; the ONLY thing left un-stamped was the terminal `sluice_opened`. The
warden, after anya_testimony_given=True, kept spawning a FRESH prereq (third-party
corroboration / upstream sign-off) — self-contradicting his own persona ("a witness
testifies in person and I open the sluice"). Root cause: world-change arbitration
never saw the warden's persona/stance (only attributes; world_book was never fed in).

Fixed (commit e3001e6): pack world_book injected into arbitration & filtered by
target entity; world-change prompt now renders "the target's persona & stance"
(warden traits + his own world_book "open the sluice once an eyewitness testifies
in person"); convergence rule (c): if the authority's own stance has stated the
release condition and it is already met (prereq var True) → MUST judge success, no
inventing a new prereq not in his stance.

This run only watches: can proving run the full end-to-end
`witness ⟳FLIP → sluice_opened ⟳FLIP`.

Reuse third-run play: escort with pure-movement imperatives; soothe Anya first;
keep co-located; testimony keywords route to npc.miller_anya; then ask warden to
open the sluice and watch (c) + persona close it.
...
LOGFILE = str(OUTDIR / "proving.log")
RAWOUT = OUTDIR / "proving_raw.txt"
...
lg.info("=== prereq_convergence_test FOURTH RUN log (proving, terminal closure) ===")
...
def main():
    log(f"=== PROVING FOURTH RUN (terminal closure) pack={PACK} "
        f"tick_to={PER_TICK_TIMEOUT}s sock_to={SOCK_TIMEOUT}s ===")
    s = GameSession(PACK, save_dir="_pq4_proving", llm_backend="minimax")
    ...
    log(f"\nRESULT: sluice_opened={wv(s, 'sluice_opened')} "
        f"anya_testimony_given={wv(s, 'anya_testimony_given')}")
    out.close()
