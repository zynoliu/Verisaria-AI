"""Second-run playability-audit driver (real MiniMax) — tidebreak.

Same persistent GameSession / EngineSession queue-tail approach as run 1's
driver, but the transcript now:
  - prints a FULL relationship snapshot (raw suspicion/trust for every NPC,
    including negative dims that the protocol snapshot hides) every tick,
  - prints the per-tick DELTA in suspicion/trust for each NPC vs prior tick,
  - tags each player line PRESSURE (施压) or SINCERE (真诚) from a queue prefix
    `[P]` / `[S]` so we can correlate话术 → 关系移动,
  - keeps tracking the silent bystander 乔巡长 (npc.sergeant_qiao) every tick.

No engine edits. Live-world switches injected on an in-memory premise copy only.
Pack may be swapped via PA_PACK (run 4 uses an authored copy).
...
# NPCs we always want to track (incl. silent bystander 乔巡长)
TRACK = ["npc.teacher_mara", "npc.engineer_sen", "npc.director_lin",
         "npc.sergeant_qiao", "npc.broadcaster_orin", "npc.vendor_yu",
         "npc.child_tavi"]

def make_session():
    g = GameSession(PACK, save_dir="_pa2_saves", llm_backend="minimax")
...
# raw dims straight from the store — shows suspicion/trust even when ≤0
PREV = {}
...
def main():
    ...
    # seed PREV with opening dims so the first tick's Δ is meaningful
    for nid in TRACK:
    ...
    s0 = es.snapshot()
    w(f"# Tidebreak 可玩性盘点 · 第二跑 transcript")
    w(f"\n包: {PACK}")
