...
from verisaria.engine import arbiter as _arb_mod

# --- read-only instrumentation: record each arbiter verdict's reason + fallback flag
ARB_LAST = {}
_orig_arbitrate = _arb_mod.LLMArbiter.arbitrate
def _wrapped_arbitrate(self, action, world):
    out = _orig_arbitrate(self, action, world)
    ao = out.arbiter_output
    ARB_LAST.update(outcome=ao.outcome, reason=ao.reason,
                    confidence=ao.confidence,
                    is_fallback=getattr(ao, "is_fallback", None),
                    escort=bool(getattr(world, "escort_request", None)))
    return out
_arb_mod.LLMArbiter.arbitrate = _wrapped_arbitrate

PACK = "fixtures/content_packs/escort_proving_ground.json"
PLAYER = "player_001"
OUTDIR = ROOT / "reports" / "escort_moved_validation_second_run"
...
def run(s, action):
    ...
    if ARB_LAST:
        log(f"  ARBITER: outcome={ARB_LAST.get('outcome')} "
            f"is_fallback={ARB_LAST.get('is_fallback')} "
            f"escort={ARB_LAST.get('escort')} conf={ARB_LAST.get('confidence')}")
        log(f"  ARBITER_REASON: {ARB_LAST.get('reason')}")
    return events
